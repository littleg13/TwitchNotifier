...
float attenuation(int i, float distToLightInEC)
{
	...
	return 1.0 / (c0c1c2[i][0] * 0.0005 + c0c1c2[i][1]*distToLightInEC + c0c1c2[i][2]*distToLightInEC*distToLightInEC);
}
...
void phong(in vec3 Q, in vec3 nHat, out vec3 ambient, out vec3 diffuse, out vec3 specular)
{
	...
	for (int lsi=0 ; lsi<actualNumLights ; lsi++)
	{
		if (lightEnabled[lsi])
		{
			...
			if (ecLightPosition[lsi].w > 0.0) // positional source in scene
			{
				vec3 li = ecLightPosition[lsi].xyz/ecLightPosition[lsi].w - Q;
				distToLightInEC = length(li); // distance Q to i-th light source
				liHat = li/distToLightInEC;
			}
			else // directional light
				liHat = normalize(ecLightPosition[lsi].xyz);
